...
CONVERSATION_DB_PATH = Path(
    __file__).resolve().parent / "src" / "db" / "conversation_history" / "./conversation_history.db"
...
def agent_search(query: str, num_image_search: int = 3, num_keyword_search: int = 3,
                 search_function=hybrid_search) -> str:
    """
    ...
    """
    return search(query=query, num_image_search=num_image_search, num_keyword_search=num_keyword_search,
                  search_function=search_function)


agent = get_shopping_agent(agent_search, conversation_db_path=CONVERSATION_DB_PATH, session_name=SESSION_NAME,
                           user_id=USER_ID)
...
async def query_images_with_text(search_params: SearchParams) -> ImageURLs:
    res = hybrid_search.hybrid_search(search_params.query, search_params.qdrant_limit, search_params.mieli_limit,
                                      [i.lower() for i in search_params.category_names], search_params.price_low,
                                      search_params.price_high)
    return ImageURLs(image_search_results=[i.images[0] for i in res.qdrant_products],
                     keyword_search_results=[i.images[0] for i in res.mieli_products])
